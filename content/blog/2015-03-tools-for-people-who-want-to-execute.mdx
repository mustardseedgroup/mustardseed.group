...
There is also a third dimension of constraint that rarely gets named: cognitive load. The over-extended executor is not just short on time and money. They are short on the kind of focused attention that produces quality decisions. Every hour spent wrestling with a tool that wasn't designed for them is an hour not spent on the actual judgement calls that determine whether their work succeeds or fails. That cost is invisible in product metrics and almost never appears in user research, but it is real and it compounds. Tools designed for the wrong person do not just waste time — they deplete the very thing that makes capable people capable.
...
There is also a failure mode that infrastructure thinking helps avoid: the tendency to solve the symptom rather than the cause. When someone tells you their biggest problem is that they can't keep track of their tasks, the instinct is to build a better task manager. But in many cases the real problem is that they have no reliable way of knowing which tasks matter most right now, which depend on something else, and which can safely wait. A better list does not address that. A surface that organises work by its actual dependencies and consequences might. The difference requires knowing your user well enough to ask the harder question.
...
The irony is that these workarounds are often more effective than the tools they supplement. A well-constructed personal spreadsheet, built around the actual shape of the work, often outperforms a feature-rich platform that wasn't designed with that shape in mind. The lesson is not that spreadsheets are better than software. It is that design specificity matters more than feature count. The person making the spreadsheet is, without framing it this way, designing a tool for themselves with a clarity that most software teams never achieve because they are not willing to commit to a specific user.
...
None of this requires sophisticated technology. It requires an honest understanding of who the user is and what they actually need. The sophistication, if it belongs anywhere, belongs in the quality of that understanding.
...
The portfolio grows from that thesis, not from a list of markets we decided to enter. Products get added when we find a meaningful gap: a place where the right person is being underserved in a way we know how to address. Products that don't belong to the thesis don't belong in the portfolio. That discipline matters. It is easy to accumulate projects. It is harder to maintain the thread of an argument across everything you build. We want the portfolio to be legible — to someone looking at it from the outside and asking what this institution is actually for, the answer should be clear.
...
These are not aspirational statements. They are constraints that drive real decisions at the product level — what to build first, what to cut, what to leave out entirely until the core earns its place. They are also the kind of constraints that expose whether a team genuinely knows its user or is simply claiming to. Anyone can write a user persona. Fewer teams will make a costly product decision because their specific user genuinely cannot afford the overhead. The willingness to cut is the test.
...
That last phrase matters. This is not charity. It is not a social mission to serve the underdog. It is a belief that the right user, served well, is a durable business. The motivated executor who finds a tool that actually works for them does not churn. They integrate it. They defend it. They do not ask whether there is something cheaper. They ask whether they can do more with it.

Building for that person, rather than for a mass market, means accepting a smaller initial audience. It also means building something that actually holds, rather than something that attracts attention briefly and then loses it to the next thing that promises more.
