...
The portfolio is an architecture. Each piece earns its place by doing its particular kind of work well. The goal is not integration for its own sake, but that each track, excellent in its own mode, makes the others better by existing alongside them.
