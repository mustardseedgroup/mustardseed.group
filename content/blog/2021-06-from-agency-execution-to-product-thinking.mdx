...
For Mustard Seed Group, this is not accidental. The portfolio is structured so that TUXX's services work is adjacent to Orbit's product development. The adjacency is the point. Not so that the two are indistinguishable — they are clearly distinct in their commercial structure and their accountability — but so that the learning flows in both directions. Orbit informs what TUXX can offer clients; TUXX informs what Orbit needs to become.
