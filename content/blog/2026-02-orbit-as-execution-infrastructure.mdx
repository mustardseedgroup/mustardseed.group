...
**Reliability without conditions.** A nice-to-have tool is allowed to be flaky. People route around it. Infrastructure does not get that tolerance. When the system that holds your commercial pipeline goes down, or returns inconsistent state, or quietly drops context, the damage is not inconvenience: it is operational degradation. The bar for uptime, data integrity, and consistency is higher for load-bearing systems than it is for supplemental ones. That bar has to be designed in, not bolted on.
