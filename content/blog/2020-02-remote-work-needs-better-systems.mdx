...
What software needs to do — genuinely needs to do, not just market as doing — is make the state of work visible without requiring a human intermediary to synthesise it. Where are we on this? Who is next? What has changed since yesterday? What is blocked and why? These should be answerable by looking at the system, not by sending a message.
