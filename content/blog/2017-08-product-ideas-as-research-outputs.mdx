...
There is a standard complaint about research: that it sits on a shelf. Papers are published, reports circulate within institutions, findings get cited in grant applications — and very little of it reaches anything a person actually uses. The critique is fair but it misidentifies the problem. The issue is not that research is inert. The issue is that most people who read it do not know how to read it as a source of product imagination.
...
This is almost always wrong. Not because summarisation is not useful, but because the research finding is an existence proof — it shows that something is possible — not a specification for how it should be experienced. The gap between "this can be done" and "this is valuable in use" is where most product work actually lives.
